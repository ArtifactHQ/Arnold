...
**Command format note:** In Claude Code plugin mode, Arnold commands are invoked as `/arnold:spec`, `/arnold:plan`, etc. In other tools using Agent Skills, they may be invoked as `/arnold-spec`, `/arnold-plan`, etc. The functionality is identical regardless of invocation format.

<context>
Docs folder contents:
!`ls -la docs/ 2>/dev/null || echo "No docs/ folder found"`

!`find docs/ -name "*.md" -type f 2>/dev/null || echo "No markdown files found"`
</context>
